...
BYTE GetFirmkeyID(BYTE keyno)
{
    ...
    for (i = 0; i < FIRMKEYS; i++)
    {
		...
		#ifndef WISE158
		if(keyno == 0x2d) //jdb2018-04-08 修改00为DEL键
		{
			return ID_DELETE;
		}
		#else
		if(keyno == 0x31) //jdb2018-04-18 WISE158
		{
			return ID_DELETE;
		}
		#endif
...
#if (defined(TCPIP) || defined(CASE_GPRS) || defined(ISP_NET)) //jdb2018-05-03 加入ISP接口网络芯片
            ProgIP();    //ccr chipcard 2004-06-28
            //jdb2018-05-03
            #ifdef ISP_NET
            Ethernet_Start(ApplVar.IP.IPAddress[0],ApplVar.IP.IPAddress[1],ApplVar.IP.IPAddress[2],ApplVar.IP.IPAddress[3]);
			#endif
#else
            ProgLineMes[0]=0;
            DispStrXY(Msg[BAOLIU].str,0,0);
