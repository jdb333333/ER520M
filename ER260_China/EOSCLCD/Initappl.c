...
//jdb2018-08-02 写入报表数据
void WriteReport(struct TOTAL Total, struct REPSIZE Size)
{
    WriteRam((BYTE*)&Total.Qty, Size.Qty + 1);
    if (Size.Cc)
		WriteRam(Total.Cc.Value, Size.Cc);
    if (Size.Amt){
		WriteRam(Total.Amt.Value, Size.Amt);
    }
    if (Size.RetQty){
		WriteRam(Total.RetQty.Value, Size.RetQty);
    }
    if (Size.Disc)
		WriteRam(Total.Disc.Value, Size.Disc);
    if (Size.Cost){
		WriteRam(Total.Cost.Value, Size.Cost);
    }
}

//jdb2018-08-02 写入GRANDTOTAL
static int WriteGrandTotal(struct TOTAL Total)
{
	struct REPSIZE Size;
	
	RamOffSet = ApplVar.AP.StartAddress[AddrTotl];
	Size = ApplVar.AP.Sales.Size[0];
	WriteReport(Total, Size);
}

//jdb2018-08-02 恢复销售数据
static void Restore_Sales_Data()
{
	struct TOTAL Total;
	BCD tmp1;
				...
				mClearScreen();
				mSetInvAttr();
				DispStrXY(RECOVRECPARAM,0,SCREENLN-1);
				mClearInvAttr();
				...
				if (Getch()==Default.FirmKeys[ID_ENTER]){
					...
					DispStrXY("REFUND#",0,1);
					DispStrXY("Z-COUNT#",0,2);
					DispStrXY("INPUT GRAND TOTAL",0,3);
	
					//反显
					mSetInvAttr();
					DispStrXY("RECEIPT#",0,0);
					mClearInvAttr();
					//BCDValueToULong(ApplVar.ReceiptNumber, &ApplVar.LNumberEntry);
					ApplVar.LNumberEntry = 0;
					DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-8,0,8);
					SETBIT(ApplVar.INPUTFUN, BIT1);
					InputWord(SCREENWD-8,0,99999999);
					RESETBIT(ApplVar.INPUTFUN, BIT1);
					memcpy(ApplVar.ReceiptNumber, ApplVar.Entry.Value, RECNUM);
					memcpy(ApplVar.ZReceiptNumber, ApplVar.Entry.Value, RECNUM);
					//不反显
					DispStrXY("RECEIPT#",0,0);
					//BCDValueToULong(ApplVar.ReceiptNumber, &ApplVar.LNumberEntry);
					DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-8,0,8);
					...
					mClearInvAttr();
					ApplVar.LNumberEntry = 0;
					DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-8,1,8);
					SETBIT(ApplVar.INPUTFUN, BIT1);
					InputWord(SCREENWD-8,1,99999999);
					RESETBIT(ApplVar.INPUTFUN, BIT1);
					memcpy(ApplVar.ReturnNumber, ApplVar.Entry.Value, RETNUM);
					//不反显
					DispStrXY("REFUND#",0,1);
					//BCDValueToULong(ApplVar.ReturnNumber, &ApplVar.LNumberEntry);
					DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-8,1,8);
					...
					mClearInvAttr();
					ApplVar.LNumberEntry = 0;
					DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-8,2,8);
					SETBIT(ApplVar.INPUTFUN, BIT1);
					InputWord(SCREENWD-8,2,99999999);
					RESETBIT(ApplVar.INPUTFUN, BIT1);
					...
					if (Getch()==Default.FirmKeys[ID_ENTER]){
						mClearScreen();
						DispStrXY("ENDING BALANCE",0,0);
						mSetInvAttr();
						DispStrXY("VALUE#",0,1);
						mClearInvAttr();
						ApplVar.LNumberEntry = 0;
						DisplayDecXY(ApplVar.LNumberEntry,SCREENWD-10,1,10);
						SETBIT(ApplVar.INPUTFUN, BIT2);
						InputWord(SCREENWD-8,1,99999999);
						RESETBIT(ApplVar.INPUTFUN, BIT2);

						memset(&Total, 0, sizeof(Total));
						SETBIT(Total.Qty.Sign, BIT2);
						Total.RetQty = ApplVar.Entry;
						//if(Total.RetQty.Sign == 0)
						{
							tmp1 = HUNDRED;
							Multiply(&Total.RetQty, &tmp1);
							Total.RetQty.Sign = 2;
						}
...
*/
    ...
    //	    InitConfig(&ApplVar.AP.Config, sizeof(struct CCONFIG));  /* Set Configuration */
			...
			//InitDept(true);	DispCharXY(sCmp,i++,2); //jdb2018-05-22
