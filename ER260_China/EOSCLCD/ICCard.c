...
#endif
		//jdb20180307
		VIPADDR = FLASH_ECRVIPFIX_ADDR+vipNumber*sizeof(struct  TECRVIPFix);
...
#define sDispBuff ProgLineMes
    ...
    //char sDispBuff[SCREENWD+1];
        ...
        if (i<ECRVIPMAX)
        {
			...
			//jdb2018-03-08
			if(ApplVar.Key.Code == SCLOGIN){
				if(strcmp(ECRVIP.VipFix.VipName, MsgSCNAME) != 0){
					strncpy(sDispBuff, MsgNOSC, sizeof(MsgNOSC));
					sDispBuff[SCREENWD]=0;
					DispStrXY(sDispBuff,0,0);
					SETBIT(ECRVIP.ECRVIPState,VIP_ERR);//表示会员登录出错
					return;
				}
			}
			if(ApplVar.Key.Code == PWDLOGIN){
				if(strcmp(ECRVIP.VipFix.VipName, MsgPWDNAME) != 0){
					strncpy(sDispBuff, MsgNOPWD, sizeof(MsgNOPWD));
					sDispBuff[SCREENWD]=0;
					DispStrXY(sDispBuff,0,0);
					SETBIT(ECRVIP.ECRVIPState,VIP_ERR);//表示会员登录出错
					return;
			...
			//jdb2018-03-08
            if(strcmp(ECRVIP.VipFix.VipName, MsgSCNAME) == 0 || strcmp(ECRVIP.VipFix.VipName, MsgPWDNAME) == 0)
            {
            	//减税百分比
                memset(sDispBuff,' ',sizeof(sDispBuff));
                CopyFrStr(sDispBuff,cVIPLESSVAT);
                sprintf(sDispBuff+SCREENWD-3,"%02d%%", 12);
                DispStrXY(sDispBuff,0,1);

				//折扣百分比
                memset(sDispBuff,' ',sizeof(sDispBuff));
                CopyFrStr(sDispBuff,cVIPDISC);
                sprintf(sDispBuff+SCREENWD-3,"%02d%%", 20);
                DispStrXY(sDispBuff,0,2);

				SETBIT(ECRVIP.ECRVIPState,VIP_SCPWD);//表示SC PWD登录
            }
            else{
    ...
    if (Appl_EntryCounter)
    {
        ...
        {//有会员
            ...
            {//指定会员打印格式
				...
				//jdb2018-03-08
				if(TESTBIT(ECRVIP.ECRVIPState, VIP_ERR))
					RESETBIT(ECRVIP.ECRVIPState, VIP_ERR);
                else
        ...
        {//增加新会员,输入会员名称
			...
			//jdb2018-03-08
            if(ApplVar.Key.Code == SCLOGIN || ApplVar.Key.Code == PWDLOGIN){
                memset(ECRVIP.VipFix.VipName, 0, sizeof(ECRVIP.VipFix.VipName));
                if(ApplVar.Key.Code == SCLOGIN)
                    strncpy(ECRVIP.VipFix.VipName, MsgSCNAME, sizeof(MsgSCNAME));
                else
                    strncpy(ECRVIP.VipFix.VipName, MsgPWDNAME, sizeof(MsgPWDNAME));
                AddNewECRVIP();
