...
//#define DOTPRNPORT 0	//testonly
	...
	while (1)
    {
    	//jdb2018-05-02 只检测打印机存在或缺纸两种状态
    	ChkS = GetCTS(DOTPRNPORT+1);
		if(ChkS == 0){
			break;
		}
        ...
        CheckError(0); //jdb2018-05-02 不报警和显示
    }
    if (ApplVar.ErrorNumber==ERROR_ID(CWXXI39))
    ...
    if (chkState==0)
    {//发送检测打印纸的命令
    	#ifdef FX330//jdb2018-08-09 FX330打印机
		ApplVar.ErrorNumber = 0;//jdb2018-08-09 错误序号清零
		SendComm(DOTPRNPORT, DLE);
		SendComm(DOTPRNPORT, EOT);
		SendComm(DOTPRNPORT, NCMD);
		#else
    ...
    if (mode==1)//为实时检测,不等待打印机应答
    {
        ...
        {
        	#ifdef FX330 //jdb2018-08-09 FX330打印机
			if (TESTBIT(ret, BIT5) || TESTBIT(ret, BIT6))//打印机无纸
			#else
            if (ret==0x0c)//打印机无纸
            #endif
    ...
    else if (mode==2)//为实时检测,必须等待打印机应答并立即报错
    {
        do
        {
            ...
            {
                EmptyComm(DOTPRNPORT);
				#ifdef FX330//jdb2018-08-09 FX330打印机查询状态命令
				ApplVar.ErrorNumber = 0;//jdb2018-08-09 错误序号清零
				SendComm(DOTPRNPORT, DLE);
				SendComm(DOTPRNPORT, EOT);
				SendComm(DOTPRNPORT, NCMD);
            ...
            if (ApplVar.ErrorNumber==ERROR_ID(CWXXI40) || ApplVar.ErrorNumber==ERROR_ID(CWXXI38))
            {
                ...
                CheckError(0); //jdb2018-05-02 重新发送检测命令检测打印机状态不需要显示和报警
            }
            else
                break;
        } while (true);
    }
	else if (mode==3)//为实时检测,必须等待打印机应答，不报错
    {
        do
        {
            if (chkState==0)
            ...
            if (ApplVar.ErrorNumber==ERROR_ID(CWXXI40) || ApplVar.ErrorNumber==ERROR_ID(CWXXI38))
            {
				...
				#if 1 //jdb2018-08-06 打印机无纸
				if(ApplVar.ErrorNumber==ERROR_ID(CWXXI40))
				{
					CheckError(0);
				}
				#else
