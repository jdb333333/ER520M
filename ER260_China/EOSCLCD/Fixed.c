...
void PrintNumber(BCD *num)
{
    ...
    temp = *num;
    temp.Sign &= 0x03;
	if(TESTBIT(ApplVar.ECRRGFLAG, BIT0))//jdb2018-07-26 输入要退货的单号
		str = FormatQtyStr(REFNUM, &temp, PRTLEN);
	else
...
//????            ApplVar.RGRec.Key.Code = 0;
	...
	//jdb2018-03-19
	case VATEXEMPT:
		//if(ApplVar.FSub || ApplVar.RGRec.Key.Code==SUB || TESTBIT(ECRVIP.ECRVIPState, BIT4)){ 		//jdb2018-03-27
			//if(!CheckNotZero(&ApplVar.LessVatTotalAmt)) //jdb2018-04-03
		if(ApplVar.FZV == 0x00 || TESTBIT(ApplVar.FZV, BIT0)){
		//ApplVar.FZV = 0x01; //jdb2018-04-24 增加标志位区分免税和零税
		SETBIT(ApplVar.FZV, BIT0);//jdb2018-04-24 增加标志位区分免税和零税
		SETBIT(ApplVar.FZV, BIT3);//jdb2018-05-02 增加按了免税或零税键标志
		}else{ //jdb2018-04-24 零税，一笔销售零税或免税只能存在一种情况
			ApplVar.ErrorNumber=ERROR_ID(CWXXI01);
			break;
		}
		
		SaveQty = ApplVar.Qty;
		if(CheckNotZero(&ApplVar.SaleAmt)){ //jdb2018-04-03
				SETBIT(ECRVIP.ECRVIPState, BIT4);
				CalculateTax(2);
				CalculateTax(5);
				DispSubTotal(); //jdb2018-03-27
				//jdb2018-04-08
				ApplVar.BufCmd = 5;
				StoreInBuffer();
		}else{ 
	...
	//jdb2018-04-24 零税销售
	case ZERORATED:
		if(ApplVar.FZV == 0x00 || TESTBIT(ApplVar.FZV, BIT1)){
		//ApplVar.FZV = 0x02;
		SETBIT(ApplVar.FZV, BIT1);
		SETBIT(ApplVar.FZV, BIT3);//jdb2018-05-02 增加按了免税或零税键标志
		}else{ //jdb2018-04-24 零税，一笔销售零税或免税只能存在一种情况
			ApplVar.ErrorNumber=ERROR_ID(CWXXI01);
			break;
