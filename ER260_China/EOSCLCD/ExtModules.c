...
//jdb2018-03-09
UINT64  HexToLong64(BYTE  *p, BYTE BcdBytes)       /* (void *p, UnLong *Lp) max is 999.999.999 */
{
    short       i;
    UINT64     re=0;

    for (i=BcdBytes-1;i>=0;i--)
    {
     	re=re*100+BCDtoDEC(p[i]);
    }
    return(re);
...
void ReadHeader()
{
	...
	if(ListRecord.Number < 8){
    	strcpy(ListRecord.Name,ApplVar.TXT.Header[ListRecord.Number]);
	}else if(ListRecord.Number < HeaderMax){ //jdb2018-03-22 票尾支持到40行
		//strcpy(ListRecord.Name,ApplVar.AddHeader[ListRecord.Number - 8]);
		strcpy(ListRecord.Name,ApplVar.TXT.Header[ListRecord.Number]);
	}
}
void ReadTrail()
{
	if(ListRecord.Number < 6){
    	strcpy(ListRecord.Name,ApplVar.TXT.Trailer[ListRecord.Number]);
	}else if(ListRecord.Number < TrailerMax){ //jdb2018-03-22 票尾支持到40行
		//strcpy(ListRecord.Name,ApplVar.AddTrailer[ListRecord.Number - 6]);
		strcpy(ListRecord.Name,ApplVar.TXT.Trailer[ListRecord.Number]);//jdb2018-03-26
	}
}
void ReadSlipHeader()
{
...
void ReadFlagsList()
{
    ...
    if (ListRecord.Number<SYSUSED)
    {
        ...
        else
        {
			...
			#if 1
			if(sIdx == 0x0e) //jdb2018-07-12 显示商铺名
			{
				memcpy(ListRecord.Name + SCREENWD - strlen(ApplVar.AP.SHOPNAME), ApplVar.AP.SHOPNAME, strlen(ApplVar.AP.SHOPNAME));
			}
			else
			#endif
...
#endif
		{SCLOGIN ,SCLOGIN	        ,MsgSCLOGIN  ,0},
		{PWDLOGIN ,PWDLOGIN	        ,MsgPWDLOGIN  ,0},
        {0        ,0                ,MsgKEYNULL   ,0}
...
BYTE InputWord(BYTE x,BYTE y, unsigned int vMax)//jdb2018-07-27 支持无符号整型
{
    WORD keyCode;
    BYTE keyNo;
	BYTE AsciiKeyMode = 0x0; //jdb2018-07-16 0x0 小写 0x01 大写
    ...
    while (true)
    {
        ...
        if (keyCode==CLEAR)
        {
            ...
            ClearEntry(0);
			ApplVar.LNumberEntry = 0;//jdb2018-07-27 清零
        }
        ...
        else if (ApplVar.ErrorNumber==0)
        {
            if (keyCode>='0' && keyCode<='9')
                AppendEntry(keyCode);
			else if(TESTBIT(ApplVar.INPUTFUN, BIT2) && ApplVar.DecimalPoint == 0 && keyCode=='.'){//jdb2018-08-02 输入小数点
				if(ApplVar.DecimalPoint < 3){ //限2位小数
					AppendEntry(keyCode);
					ApplVar.DecimalPoint++;
				}
			}
			else if(TESTBIT(ApplVar.INPUTFUN, BIT0)) //jdb2018-07-12 输入店铺名
			{
				keyCode = ASCIIKEY[keyNo];
				if(keyCode > 0)
				{
					if (AsciiKeyMode==0x01 && keyCode>='a' && keyCode<='z') //jdb2018-07-16 输入大写ASC
					{
						keyCode &=(~0x20);
					}
					AppendEntry(keyCode);
			...
			else if(TESTBIT(ApplVar.INPUTFUN, BIT1) //jdb2018-07-26 输入无符号整型，输入了回车键
				|| TESTBIT(ApplVar.INPUTFUN, BIT2)) //jdb2018-08-02 输入浮点数，输入了回车键
			{
				if(keyCode == TEND + 1)
					return;
				else
                ...
                return keyNo;
            GetEntry();
			if(TESTBIT(ApplVar.INPUTFUN, BIT0)) //jdb2018-07-12 输入店铺名 最大长度限制
			{
				if(Appl_EntryCounter > vMax)
					ApplVar.ErrorNumber=ERROR_ID(CWXXI18);
			}
			else
			{
			if(TESTBIT(ApplVar.INPUTFUN, BIT1)){//jdb2018-07-27 输入无符号整型
				if (ApplVar.LNumberEntry>vMax)
                	ApplVar.ErrorNumber=ERROR_ID(CWXXI18);
			}else if(TESTBIT(ApplVar.INPUTFUN, BIT2)){//jdb2018-08-02 浮点数无最大值限制
			}else{
