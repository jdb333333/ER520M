...
void ProgHeader()
{
    ...
    else if (Appl_ProgLine > 2 && Appl_ProgNumber<8)
    {
        ...
        {
            //Appl_ProgNumber = 8;
			Appl_ProgNumber = HeaderMax; //jdb2018-03-23
        }//ccr2016-01-22<<<<<<<<<
    }
	//jdb2018-03-23
	else if(Appl_ProgLine > 2 && Appl_ProgNumber<HeaderMax){
		Appl_ProgNumber++;
        Appl_ProgLine = 2;
		#if 0
		if(Appl_ProgNumber == 8 && ApplVar.TXT.Header[Appl_ProgNumber-1][0]==0){
			Appl_ProgNumber = 40;
		}
		else if (Appl_ProgNumber>8 && Appl_ProgNumber<40 && ApplVar.AddHeader[Appl_ProgNumber-1-8][0]==0)
		{
			Appl_ProgNumber = 40;
		}
		#else
		if (Appl_ProgNumber>0 && Appl_ProgNumber<HeaderMax && ApplVar.TXT.Header[Appl_ProgNumber-1][0]==0)
        {
			Appl_ProgNumber = HeaderMax;
        }
		#endif
	}
	...
	if (Appl_ProgNumber == HeaderMax) //jdb2018-03-23
    {
        Appl_MaxEntry=0;
        Appl_EntryCounter = 0;
        if (Appl_ProgLine==2)
	...
	}else if(Appl_ProgNumber < HeaderMax){//jdb2018-03-22 票头支持到40行
		#if 0
	    GetString(SETTRAIL, ApplVar.AddHeader[Appl_ProgNumber - 8], PRTLEN,Appl_ProgNumber+1);
    	ApplVar.AddHeader[Appl_ProgNumber - 8][PRTLEN]=0;
    	Appl_MaxEntry=sizeof(ApplVar.AddHeader[0]);
		#else
		GetString(SETHEAD, ApplVar.TXT.Header[Appl_ProgNumber], PRTLEN,Appl_ProgNumber+1);
		ApplVar.TXT.Header[Appl_ProgNumber][PRTLEN]=0;
		Appl_MaxEntry=sizeof(ApplVar.TXT.Header[0]);
...
void ProgTrailer()
{
    ...
    else if (Appl_ProgLine > 2 && Appl_ProgNumber<6)
    {
        ...
        {
            //Appl_ProgNumber = 6;
			Appl_ProgNumber = TrailerMax; //jdb2018-03-23
        }
    }
	//jdb2018-03-22 增加到40行
    else if (Appl_ProgLine > 2 && Appl_ProgNumber<TrailerMax)
    {
        Appl_ProgNumber++;
        Appl_ProgLine = 2;
		#if 0
		if(Appl_ProgNumber == 6 && ApplVar.TXT.Trailer[Appl_ProgNumber-1][0]==0){
			Appl_ProgNumber = 40;
		}
        else if (Appl_ProgNumber>6 && Appl_ProgNumber<40 && ApplVar.AddTrailer[Appl_ProgNumber-1-6][0]==0)
        {
            Appl_ProgNumber = 40;
        }
		#else //jdb2018-03-26
		if(Appl_ProgNumber>0 && Appl_ProgNumber<TrailerMax && ApplVar.TXT.Trailer[Appl_ProgNumber-1][0]==0){
			Appl_ProgNumber = TrailerMax;
		}
		#endif
    }
	...
	if (Appl_ProgNumber == TrailerMax) //jdb2018-03-22 增加到40行
    {
        Appl_MaxEntry=0;
        Appl_EntryCounter = 0;
        if (Appl_ProgLine==2)
	...
	}else if(Appl_ProgNumber < TrailerMax){//jdb2018-03-22 票尾支持到40行
		#if 0
	    GetString(SETTRAIL, ApplVar.AddTrailer[Appl_ProgNumber - 6], PRTLEN,Appl_ProgNumber+1);
    	ApplVar.AddTrailer[Appl_ProgNumber - 6][PRTLEN]=0;
    	Appl_MaxEntry=sizeof(ApplVar.AddTrailer[0]);
		#else //jdb2018-03-26
    	GetString(SETTRAIL, ApplVar.TXT.Trailer[Appl_ProgNumber], PRTLEN,Appl_ProgNumber+1);
    	ApplVar.TXT.Trailer[Appl_ProgNumber][PRTLEN]=0;
    	Appl_MaxEntry=sizeof(ApplVar.TXT.Trailer[0]);
...
BYTE NewProgSysFlag(BYTE y,WORD recNo)
{
    ...
    if (recNo<SYSUSED)
    {
        ...
        else
        {
			...
			if(sIdx == 0x0e) //jdb2018-07-12 输入店铺名
			{
				SETBIT(ApplVar.INPUTFUN, BIT0);
			}
			else
...
//                DispCharXY(ApplVar.NumberEntry,SCREENWD-5,y);
			...
			if(sIdx == 0x0e) //jdb2018-07-12 输入店铺名 字长10
			{
				updownkey = InputWord(SCREENWD-10,y,10);
			}
			else
            updownkey = InputWord(SCREENWD-3,y,SysFlagUsed[recNo].Max);

			if(sIdx == 0x0e) //jdb2018-07-12 保存店铺名
			{
				RESETBIT(ApplVar.INPUTFUN, BIT0);
				if(Appl_EntryCounter > 0)
				{
					memset(ApplVar.AP.SHOPNAME, 0, sizeof(ApplVar.AP.SHOPNAME));
					memcpy(ApplVar.AP.SHOPNAME, EntryBuffer + ENTRYSIZE - 1 - Appl_EntryCounter, Appl_EntryCounter);
				}
			}
			else
