...
//cc 2006-10-19 for SubDisc<<<<<<<<<<<<<<<<<<
	    ...
	    else
	    {
	    	if(!TESTBIT(ApplVar.ECRRGFLAG, BIT0)){
				...
				SETBIT(ApplVar.ECRRGFLAG, BIT0);//jdb2018-07-26 退货标志置一
				mClearScreen();
				DispStrXY(ClearFlashChar(ApplVar.Correc.Name,SCREENWD),0,0);
				DispStrXY(INPUTREFNUM, 0, 1);
				SETBIT(ApplVar.INPUTFUN, BIT1);
	    		InputWord(SCREENWD-8,1,99999999);
				RESETBIT(ApplVar.INPUTFUN, BIT1);
				//PrintNumber(&ApplVar.Entry);
				ApplVar.REFECRNUM = ApplVar.Entry;
	    	}
