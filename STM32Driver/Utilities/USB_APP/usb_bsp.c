...
// EXTI_InitTypeDef EXTI_InitStructure;
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9  |		   //ccr GPIO_Pin_8
                                GPIO_Pin_11 |
                                GPIO_Pin_12;
  ...
  GPIO_PinAFConfig(GPIOA,GPIO_PinSource9,GPIO_AF_OTG1_FS) ;	//ccr VBUS GPIO_PinSource8
...
void USB_OTG_BSP_uDelay (const uint32_t usec)
{
  ...
  usDelay(usec);//ccr BSP_Delay(usec,TIM_USEC_DELAY);
#else
...
{
#ifdef USE_ACCURATE_TIME
    msDelay(msec);//ccr BSP_Delay(msec,TIM_MSEC_DELAY);
#else
