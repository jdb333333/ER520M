...
SD_Error SD_Init(void)
{
  ...
  //ouhs 20140414 SD_LowLevel_Init();

  SDIO_DeInit();

  SD_LastError = SD_PowerON();

  if (SD_LastError != SD_OK)
  {
    /*!< CMD Response TimeOut (wait for CMDSENT flag) */
    return(SD_LastError);
  }

  SD_LastError = SD_InitializeCards();
  ...
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;  //ouhs 20140814 SDIO_HardwareFlowControl_Disable

  SDIO_Init(&SDIO_InitStructure);

  /*----------------- Read CSD/CID MSD registers ------------------*/
  SD_LastError = SD_GetCardInfo(&SDCardInfo);

  if (SD_LastError == SD_OK)
  {
    /*----------------- Select Card --------------------------------*/
    SD_LastError = SD_SelectDeselect((uint32_t) (SDCardInfo.RCA << 16));
  }

  if (SD_LastError == SD_OK)
  {
    SD_LastError = SD_EnableWideBusOperation(SDIO_BusWide_1b);  //ccr2014-08-12// 开启4位模式
  }
...
SD_Error SD_PowerON(void)
{
  ...
  SD_LastError = CmdError();	 // 检测是否正确接收到CMD0
  ...
  /*!< CMD Response: R7 */
  SDIO_CmdInitStructure.SDIO_Argument = SD_CHECK_PATTERN;	   // SD卡接收到命令会返回这个参数
  ...
  SD_LastError = CmdResp7Error();

  if (SD_LastError == SD_OK)										// 有响应,说明卡遵循SD V2.0	,并默认设置为SDSC
  {
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0; /*!< SD Card 2.0 */
    SDType = SD_HIGH_CAPACITY;									// 用于询问是SDSC还是SDHC
  }
  else															// 没有响应,说明是1.x或MMC卡
  {
    /*!< CMD55 */
    ...
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
    SD_LastError = CmdResp1Error(SD_CMD_APP_CMD);
  }
  /*!< CMD55 */													 // 发送CMD55用于判断是1.x还是MMC卡或2.0（）
  SDIO_CmdInitStructure.SDIO_Argument = 0x00;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  ...
  /*!< If SD_LastError is Command TimeOut, it is a MMC card */
  /*!< If SD_LastError is SD_OK it is a SD card: SD card 2.0 (voltage range mismatch)
     or SD card 1.x */
  if (SD_LastError == SD_OK)	//SD Card
  {
    /*!< SD CARD */
    /*!< Send ACMD41 SD_APP_OP_COND with Argument 0x80100000 */
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))	  // 循环发送sdio支持的电压范围
    {
	  // 发送ACMD命令前需要先发送CMD55命令
	  ...
	  // ACMD41命令由支持的电压范围和HCS位组成，HCS位置用于区别是SDHC还是SDSC
      ...
      SD_LastError = CmdResp3Error();
      if (SD_LastError != SD_OK)
      {
        return(SD_LastError);
      }
	  // 若卡的需求电压在SDIO供电电压范围内，会自动上电并标志pwr_up位
      response = SDIO_GetResponse(SDIO_RESP1);
      validvoltage = (((response >> 31) == 1) ? 1 : 0);		// 读取卡OCR寄存器pwr_up位看是否已工作在正常电压
      count++;
    }
    if (count >= SD_MAX_VOLT_TRIAL)							// SDIO不支持卡的供电电压
    {
      SD_LastError = SD_INVALID_VOLTRANGE;
      return(SD_LastError);
    }

    if (response &= SD_HIGH_CAPACITY)						// 读取卡OCR寄存器CCS位
    {
      CardType = SDIO_HIGH_CAPACITY_SD_CARD;				// 把卡类型由初始化的SDSC型改为SDHC型
    }
...
SD_Error SD_InitializeCards(void)
{
  ...
  {
    SD_LastError = SD_REQUEST_NOT_APPLICABLE;
    return(SD_LastError);
  }

  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)	  // 判断卡类型
  {
    ...
    SD_LastError = CmdResp2Error();

    if (SD_OK != SD_LastError)
    {
      return(SD_LastError);
    }

    CID_Tab[0] = SDIO_GetResponse(SDIO_RESP1);	  // 读取卡的CID寄存器内容
      ...
      ||  (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    SD_LastError = CmdResp6Error(SD_CMD_SET_REL_ADDR, &rca);	   // 获取卡的相对地址
  ...
  SD_LastError = SD_OK; /*!< All cards get intialized */
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  else if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    ...
    //ouhs 20140701 cardinfo->CardCapacity = (cardinfo->SD_csd.DeviceSize + 1) * 512 * 1024;
...
SD_Error SD_GetCardStatus(SD_CardStatus *cardstatus)
{
  ...
  SD_LastError = SD_OK;
  SD_LastError = SD_SendSDStatus((uint32_t *)SDSTATUS_Tab);
...
SD_Error SD_EnableWideBusOperation(uint32_t WideMode)
{
  ...
  {
    SD_LastError = SD_UNSUPPORTED_FEATURE;
    return(SD_LastError);
  }
  else if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    {
      SD_LastError = SDEnWideBus(ENABLE);
    ...
    {
      SD_LastError = SDEnWideBus(DISABLE);
...
SD_Error SD_SelectDeselect(uint32_t addr)
{
  ...
  SD_LastError = CmdResp1Error(SD_CMD_SEL_DESEL_CARD);
...
//<<<<<<<ouhs 20140701
  ...
  {
#if BLOCK512
    BlockSize = SDBLOCKSIZE;
    ReadAddr /= SDBLOCKSIZE;
#else// ouhs
    BlockSize = SDCardInfo.CardBlockSize;
    ReadAddr /= SDCardInfo.CardBlockSize;
#endif
  }
  ...
  SD_LastError = CmdResp1Error(SD_CMD_SET_BLOCKLEN);
  ...
  SD_LastError = CmdResp1Error(SD_CMD_READ_SINGLE_BLOCK);
  ...
  if (SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET)
  {
    ...
    SD_LastError = SD_DATA_TIMEOUT;
    return(SD_LastError);
  }
  else if (SDIO_GetFlagStatus(SDIO_FLAG_DCRCFAIL) != RESET)
  {
    SDIO_ClearFlag(SDIO_FLAG_DCRCFAIL);
    SD_LastError = SD_DATA_CRC_FAIL;
    return(SD_LastError);
  }
  else if (SDIO_GetFlagStatus(SDIO_FLAG_RXOVERR) != RESET)
  {
    SDIO_ClearFlag(SDIO_FLAG_RXOVERR);
    SD_LastError = SD_RX_OVERRUN;
    return(SD_LastError);
  }
  else if (SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET)
  {
    SDIO_ClearFlag(SDIO_FLAG_STBITERR);
    SD_LastError = SD_START_BIT_ERR;
    return(SD_LastError);
  }
...
#if defined (SD_DMA_MODE) //ouhs 20140814
...
//<<<<<<<<ouhs 20140701
  ...
  SD_LastError = CmdResp1Error(SD_CMD_READ_MULT_BLOCK);
...
SD_Error SD_WaitReadOperation(void)
{
  ...
  if (StopCondition == 1)
  {
    SD_LastError = SD_StopTransfer();
    StopCondition = 0;
  }

  if ((timeout == 0) && (SD_LastError == SD_OK))
  {
    SD_LastError = SD_DATA_TIMEOUT;
  }
...
//<<<<<<<<<ouhs 20140701
...
#if BLOCK512
    BlockSize = SDBLOCKSIZE;
    WriteAddr /= SDBLOCKSIZE;
#else// ouhs
    BlockSize = SDCardInfo.CardBlockSize;
    WriteAddr /= SDCardInfo.CardBlockSize;
#endif
  }
  ...
  SD_LastError = CmdResp1Error(SD_CMD_WRITE_SINGLE_BLOCK);
...
//<<<<<<ouhs 20140701
  ...
  SD_LastError = CmdResp1Error(SD_CMD_SET_BLOCK_COUNT);
  ...
  SD_LastError = CmdResp1Error(SD_CMD_WRITE_MULT_BLOCK);
...
SD_Error SD_StopTransfer(void)
{
  ...
  SD_LastError = CmdResp1Error(SD_CMD_STOP_TRANSMISSION);
...
SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
  ...
  {
    SD_LastError = SD_LOCK_UNLOCK_FAILED;
    return(SD_LastError);
  }

  if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    startaddr /= SDBLOCKSIZE;
    endaddr /= SDBLOCKSIZE;
  }
  ...
  if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    SD_LastError = CmdResp1Error(SD_CMD_SD_ERASE_GRP_START);
    if (SD_LastError != SD_OK)
    {
      return(SD_LastError);
    }
    ...
    SD_LastError = CmdResp1Error(SD_CMD_SD_ERASE_GRP_END);
    if (SD_LastError != SD_OK)
    {
      return(SD_LastError);
    }
  ...
  SD_LastError = CmdResp1Error(SD_CMD_ERASE);
  ...
  SD_LastError = IsCardProgramming(&cardstate);
  delay = SD_DATATIMEOUT;
  while ((delay > 0) && (SD_LastError == SD_OK) && ((SD_CARD_PROGRAMMING == cardstate) || (SD_CARD_RECEIVING == cardstate)))
  {
    SD_LastError = IsCardProgramming(&cardstate);
    delay--;
  }
...
SD_Error SD_SendStatus(uint32_t *pcardstatus)
{
  ...
  if (pcardstatus == NULL)
  {
    SD_LastError = SD_INVALID_PARAMETER;
    return(SD_LastError);
  }
  ...
  SD_LastError = CmdResp1Error(SD_CMD_SEND_STATUS);
...
SD_Error SD_SendSDStatus(uint32_t *psdstatus)
{
  ...
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&SDIO_CmdInitStructure);
  SD_LastError = CmdResp1Error(SD_CMD_SD_APP_STAUS);
...
static SD_Error CmdError(void)
{
  ...
  if (timeout == 0)
  {
    SD_LastError = SD_CMD_RSP_TIMEOUT;
    return(SD_LastError);
  }
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
  ...
  if (NewState == ENABLE)
  {
    ...
    if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)
    {
      ...
      SD_LastError = CmdResp1Error(SD_CMD_APP_SD_SET_BUSWIDTH);
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
  ...
  SD_LastError = CmdResp1Error(SD_CMD_SD_APP_SEND_SCR);
...
void SD_LowLevel_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = SD_DETECT_PIN;			  //ouhs20130128
...
/**
  ...
  * @retval None
  */
void NVIC_SDConfiguration(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//ouhs 20140814 = 1;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;  //ouhs 20140814 = 0;
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
  NVIC_InitStructure.NVIC_IRQChannel = SD_SDIO_DMA_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;  //ouhs 20140814 = 1;
  NVIC_Init(&NVIC_InitStructure);
}


SD_Error SD_GetLastError(void)
{
  return SD_LastError;
}
/**
