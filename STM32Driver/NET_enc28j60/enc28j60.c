...
void enc28j60Init(u8 * macaddr)
{
   ...
   //ENC28J60_RSTH();//ouhs
   ENC28J60_CSH();/* 禁止ENC28J60 */
   enc28j60WriteOp(ENC28J60_SOFT_RESET, 0, ENC28J60_SOFT_RESET);//ENC28J60软件复位
   // Del_1ms(250);
   // check CLKRDY bit to see if reset is complete
   // The CLKRDY does not work. See Rev. B4 Silicon Errata point. Just wait.
   //while(!(enc28j60Read(ESTAT) & ESTAT_CLKRDY));查询ESTAT.CLKRDY位
   // do bank 0 stuff
   // initialize receive buffer
   // 16-bit transfers, must write low byte first
   // set receive buffer start address设置接收缓冲区起始地址 该变量用于每次读取缓冲区时保留下一个包的首地址
   NextPacketPtr = RXSTART_INIT;
   // Rx start设置接收缓冲区 起始指针
   enc28j60Write(ERXSTL, RXSTART_INIT & 0xFF);
   enc28j60Write(ERXSTH, RXSTART_INIT >> 8);
   // set receive pointer address设置接收缓冲区 读指针
   enc28j60Write(ERXRDPTL, RXSTART_INIT & 0xFF);
   enc28j60Write(ERXRDPTH, RXSTART_INIT >> 8);
   // RX end设置接收缓冲区 结束指针
   enc28j60Write(ERXNDL, RXSTOP_INIT & 0xFF);
   enc28j60Write(ERXNDH, RXSTOP_INIT >> 8);
   // TX start设置发送缓冲区 起始指针
   enc28j60Write(ETXSTL, TXSTART_INIT & 0xFF);
   enc28j60Write(ETXSTH, TXSTART_INIT >> 8);
   // TX end设置发送缓冲区 结束指针
   enc28j60Write(ETXNDL, TXSTOP_INIT & 0xFF);
   enc28j60Write(ETXNDH, TXSTOP_INIT >> 8);
   // do bank 1 stuff, packet filter:使能单播过滤 使能CRC校验 使能 格式匹配自动过滤
   ...
   //
   // do bank 2 stuff
   // enable MAC receive使能MAC接收 允许MAC发送暂停控制帧 当接收到暂停控制帧时停止发送
   enc28j60Write(MACON1, MACON1_MARXEN | MACON1_TXPAUS | MACON1_RXPAUS);
   // bring MAC out of reset退出复位状态
   enc28j60Write(MACON2, 0x00);
   /* 用0填充所有短帧至60字节长 并追加一个CRC 发送CRC使能 帧长度校验使能 MAC全双工使能*/
   // enable automatic padding to 60bytes and CRC operations提示 由于ENC28J60不支持802.3的自动协商机制， 所以对端的网络卡需要强制设置为全双工
   ...
   // Do not send packets longer than MAX_FRAMELEN:最大帧长度
   enc28j60Write(MAMXFLL, MAX_FRAMELEN & 0xFF);
   enc28j60Write(MAMXFLH, MAX_FRAMELEN >> 8);
   // do bank 3 stuff
   // write MAC address写入MAC地址
   ...
   // no loopback of transmitted frames半双工回环禁止
   enc28j60PhyWrite(PHCON2, PHCON2_HDLDIS);
   // switch to bank 0返回BANK0
   enc28j60SetBank(ECON1);
   // enable interrutps使能中断 全局中断 接收中断 连接中断
   enc28j60WriteOp(ENC28J60_BIT_FIELD_SET, EIE, EIE_INTIE | EIE_PKTIE | EIE_LINKIE);//ccr2015-01-31
   // enable packet reception接收使能位
