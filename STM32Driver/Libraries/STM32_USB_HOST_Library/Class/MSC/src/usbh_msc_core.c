...
USBH_Class_cb_TypeDef  USBH_MSC_Callbacks =
{
...
/**
  * 将USBH_MSC_Callbacks设置为指向USBH_MSC的回调函数
  */
//ccr>>>>>>>>>>>>>>>>>>>>
void Set_USBH_MSC_CallBacks(void)
{
    USBH_MSC_Callbacks.Init = USBH_MSC_InterfaceInit;
    USBH_MSC_Callbacks.DeInit = USBH_MSC_InterfaceDeInit;
    USBH_MSC_Callbacks.Requests = USBH_MSC_ClassRequest;
    USBH_MSC_Callbacks.Machine = USBH_MSC_Handle;
}
//<<<<<<<<<<<<<<<<<<<<<<<
