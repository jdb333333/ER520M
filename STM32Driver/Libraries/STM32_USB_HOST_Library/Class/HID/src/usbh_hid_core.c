...
extern USBH_Class_cb_TypeDef  USBH_MSC_Callbacks;//ccr

/*ccr>>>>>>>>>>>>>>
USBH_Class_cb_TypeDef  USBH_HID_Callbacks =
{
...
/**
  * 将USBH_MSC_Callbacks设置为指向USBH_HID的回调函数
*/
//ccr>>>>>>>>>>>>>>>>>>>>
void Set_USBH_HID_CallBacks(void)
{
    USBH_MSC_Callbacks.Init = USBH_HID_InterfaceInit;
    USBH_MSC_Callbacks.DeInit = USBH_HID_InterfaceDeInit;
    USBH_MSC_Callbacks.Requests = USBH_HID_ClassRequest;
    USBH_MSC_Callbacks.Machine = USBH_HID_Handle;
}
//<<<<<<<<<<<<<<<<<<<<<<<
